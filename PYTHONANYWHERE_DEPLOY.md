...
- **“获取信息”按钮提示网络请求失败**：PythonAnywhere 免费账号只能访问白名单内的外部网站，National Geographic 不在白名单内，因此“粘贴链接自动获取元信息”功能无法使用。你仍然可以手动填写标题、作者、摘要和正文，其他功能（保存、复制到 Word、下载 DOCX、打印）都正常。如果你一定要使用自动获取功能，需要换成 Render/Koyeb/Replit 等不限制出站访问的平台。
